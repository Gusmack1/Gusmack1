...
export default function SocialShare({ title, description, url, image }: SocialShareProps) {
  ...
  const shareLinks = [
    {
      name: 'Instagram',
      url: `https://www.instagram.com/?url=${encodeURIComponent(fullUrl)}`,
      icon: (
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3">
        ...
        {typeof window !== 'undefined' && 'share' in navigator && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[var(--scot-accent)] text-white hover:bg-[var(--scot-accent)]/90 rounded-lg transition-colors"
            title="Share"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="text-sm text-neutral-600">
        <p>Share this review with friends and family to help them discover great restaurants in Glasgow!</p>
      </div>
    </div>
  );
}
